...
class image_processor():
    def normalize_and_clahe(self, img, clip_limit=10.0, tile_grid_size=(8,8)):
        """
    ...
    def process_incident_illumination(self, images, output_dir=None, background_frame_idx=0, process_frame_idx=None, 
        ...
        background = cv2.rotate(
            self.normalize_and_clahe(images[background_frame_idx]), 
        ...
        for i in tqdm(frames_to_process, desc="Processing incident illumination frames"):
            ...
            frame = cv2.rotate(
                self.normalize_and_clahe(images[i]), 
                cv2.ROTATE_90_CLOCKWISE
            )
            ...
            if use_torch and torch_kernels:
                ...
                processed, contour, positions = t_processor.process_image_with_kernels(
                    frame,#diff,
                    torch_kernels,
                    threshold_value=0.4,
                ...
                clahe_img_noise = self.normalize_and_clahe(diff) #For first frame=background we will get warning since all 0s
            ...
            clahe_img = self.normalize_and_clahe(result)
            ...
            if i == len(frames_to_process) // 2 and output_dir:
                ...
                self.create_combined_image(images_list, self.display_image, save_name=combined_path)
        
        return images_list

    def process_back_illumination(self, images, output_dir=None, background_frame_idx=0, start_idx=None, end_idx=None,
        ...
        def adaptive_contrast(self, img, threshold):
        ...
        for i in tqdm(range(start_idx, end_idx), desc="Processing back illumination frames"):
            ...
            if use_torch and torch_kernels:
                ...
                processed, contour, positions = self.process_image_with_kernels(
                    image,#s[i],
                    torch_kernels,
    ...
    def display_image(self, title, img, save_path=None):
        """
    ...
    def create_combined_image(self, images_list, display_func, save_name=None):
        """
