...
def setup_columns(image_width, image_height, num_columns=5, bin_start=100, bin_end=200):
    """
...
def analyze_columns(frames_dir, num_columns=5, output_dir=None, max_frames=None, 
    ...
    column_positions = setup_columns(image_width, image_height, num_columns, bin_start, bin_end)
    # print(f"Column positions: {column_positions}")
    ...
    if output_dir:
        create_plots(bin_arrays, column_positions, frame_count, output_dir, column_width)
...
def create_plots(bin_arrays, column_positions, frame_count, output_dir, column_width=1):
    """
...
def create_animation(frames_dir, output_dir, bin_arrays, column_positions, frame_count, num_columns=5, fps=10, max_frames=None, column_width=1):
    """
...
def detect_roller_area(frame, threshold=10, illumination_type='back'):
    """
...
def get_wave_roller(frames_dir, output_dir=None, starting_frame=0, max_frames=None,
    ...
    for frame_num in tqdm(range(starting_frame, n_frames), desc="Finding wave roller start"):
        ...
        area = detect_roller_area(frame, threshold_img, illumination_type)
        ...
        if contours:
            ...
            if width > 0.999 * gray.shape[1] and flag == 0:
                ...
                if output_dir:
                    ...
                    cv2.putText(vis_frame, f"Area: {detect_roller_area(frame)}", 
